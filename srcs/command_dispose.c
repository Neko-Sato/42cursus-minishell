...
/*   Updated: 2024/03/05 22:15:33 by hshimizu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	dispose_redirect(t_redirect *redirect)
{
	...
	while (redirect)
	{
		...
		if (redirect->type != RT_HEREDOC)
			free(redirect->value.filename);
		else
		{
			free(redirect->value.document->eof);
			free(redirect->value.document->document);
			free(redirect->value.document);
		}
